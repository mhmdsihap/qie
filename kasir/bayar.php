...
$query = mysqli_query($conn, "SELECT tbl_t_transaksi.*, tbl_m_pelanggan.nama_pelanggan, tbl_m_detail_transaksi.total_harga FROM tbl_t_transaksi INNER JOIN tbl_m_pelanggan ON tbl_m_pelanggan.id_pelanggan = tbl_t_transaksi.id_pelanggan INNER JOIN tbl_m_detail_transaksi ON tbl_m_detail_transaksi.id_transaksi = tbl_t_transaksi.id_transaksi WHERE tbl_t_transaksi.id_transaksi = " . $_GET['id']);
$data = mysqli_fetch_assoc($query);

if (isset($_POST['btn-simpan'])) {
    $total_bayar = $_POST['total_bayar'];
    if ($total_bayar >= $data['total_harga']) {
        $query = "UPDATE tbl_t_transaksi SET status_bayar = 'dibayar', tgl_pembayaran = '" . date('Y-m-d h:i:s') . "' WHERE id_transaksi = " . $_GET['id'];
        $query2 = "UPDATE tbl_m_detail_transaksi SET total_bayar = '$total_bayar' WHERE id_transaksi = " . $_GET['id'];
