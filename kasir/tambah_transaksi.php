...
if (isset($_POST['btn-simpan'])) {
    ...
    if ($insert == 1) {
        ...
        $qty = $_POST['qty'];
        $query5 = mysqli_query($conn, "SELECT * FROM tbl_m_paket_cuci WHERE id_paket = $id_paket");
        $paket_harga = mysqli_fetch_assoc($query5);
        $total = $paket_harga['harga'] * $qty;
        $kode_invoice;
        $query6 = mysqli_query($conn, "SELECT * FROM tbl_t_transaksi WHERE kode_invoice = '" . $kode_invoice . "'");
        $transaksi = mysqli_fetch_assoc($query6);
        $id_transaksi = $transaksi['id_transaksi'];

        $query_detail = "INSERT INTO tbl_m_detail_transaksi (id_transaksi, id_paket, qty, total_harga) VALUES ('$id_transaksi', '$id_paket', '$qty', '$total')";
