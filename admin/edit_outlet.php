...
$query = "SELECT tbl_m_outlet.*, tbl_m_user.nama_user, tbl_m_user.id_user FROM tbl_m_outlet LEFT JOIN tbl_m_user ON tbl_m_user.outlet_id = tbl_m_outlet.id_outlet WHERE id_outlet  = " . $_GET['id'];
$data = mysqli_query($conn, $query);
$edit = mysqli_fetch_assoc($data);


$query2 = "SELECT tbl_m_user.*, tbl_m_outlet.nama_outlet FROM tbl_m_outlet RIGHT JOIN tbl_m_user ON tbl_m_user.outlet_id = tbl_m_outlet.id_outlet WHERE tbl_m_user.role = 'owner' ORDER BY tbl_m_user.outlet_id ASC";
...
if (isset($_POST['btn-simpan'])) {
    ...
    $query = "UPDATE tbl_m_outlet SET nama_outlet = '$nama', alamat_outlet = '$alamat', telp_outlet = '$telp' WHERE id_outlet = " . $_GET['id'];

    if ($_POST['owner_new_id']) {
        $query2 = "UPDATE tbl_m_user SET outlet_id = '" . $_GET['id'] . "' WHERE id_user = " . $_POST['owner_new_id'];
        $query3 = "UPDATE tbl_m_user SET outlet_id = NULL WHERE id_user = " . $edit['id_user'];
        $update3 = mysqli_query($conn, $query3);
    } else {
        $query2 = "UPDATE tbl_m_user SET outlet_id = '" . $_GET['id'] . "' WHERE id_user = " . $_POST['id_owner'];
    }
